...
"""
═══════════════════════════════════════════════════════════════
  🤖 ULTRABOT TELEGRAM - Version Render-Compatible
═══════════════════════════════════════════════════════════════
"""
...
async def cmd_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text(f"🔍 Recherche en cours\.", parse_mode=ParseMode.MARKDOWN_V2)
...
async def cmd_mute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await chat.restrict_member(target.id, until_date=until, permissions={"can_send_messages": False, "can_send_media_messages": False, "can_send_other_messages": False, "can_add_web_page_previews": False})
...
async def cmd_unmute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target = update.message.reply_to_message.from_user
    try:
        await chat.restrict_member(target.id, permissions={"can_send_messages": True, "can_send_media_messages": True, "can_send_other_messages": True, "can_add_web_page_previews": True})
...
async def cmd_privacy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "• Messages dans les groupes \(modération\)\n"
        "• Logs d\'activité\n\n"
        "*Utilisation\:*\n"
        ...
        if not check["is_clean"]:
            ...
            if action == "delete":
                ...
                try:
                    ...
                    await chat.restrict_member(user.id, until_date=until, permissions={"can_send_messages": False, "can_send_media_messages": False, "can_send_other_messages": False})
...
async def init_database():
    """Initialise la base de données avant démarrage"""
...
def main():
    ...
    # Initialisation synchrone de la DB avant de créer l'application
